// <auto-generated />
using System;
using Contexts;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace TaskTracker.Migrations
{
    [DbContext(typeof(TaskContext))]
    [Migration("20240711173154_newModel")]
    partial class newModel
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.6")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Models.Author", b =>
                {
                    b.Property<int>("AuthorId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("AuthorId"));

                    b.Property<int>("TaskAuthorUserId")
                        .HasColumnType("integer");

                    b.HasKey("AuthorId");

                    b.HasIndex("TaskAuthorUserId");

                    b.ToTable("Authors");
                });

            modelBuilder.Entity("Models.Executor", b =>
                {
                    b.Property<int>("ExecutorId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("ExecutorId"));

                    b.Property<int>("ExecutorTaskUserId")
                        .HasColumnType("integer");

                    b.HasKey("ExecutorId");

                    b.HasIndex("ExecutorTaskUserId");

                    b.ToTable("Executors");
                });

            modelBuilder.Entity("Models.Roles", b =>
                {
                    b.Property<int>("RoleId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("RoleId"));

                    b.Property<int>("UserRole")
                        .HasColumnType("integer");

                    b.HasKey("RoleId");

                    b.ToTable("Roles");
                });

            modelBuilder.Entity("Models.User", b =>
                {
                    b.Property<int>("UserId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("UserId"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("Profession")
                        .HasColumnType("integer");

                    b.Property<string>("UserEmail")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("UserId");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("Models.UserRoles", b =>
                {
                    b.Property<int>("UserRoleId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("UserRoleId"));

                    b.Property<DateTime>("LastTimeRoleChanged")
                        .HasColumnType("timestamp without time zone");

                    b.Property<int>("RoleId")
                        .HasColumnType("integer");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("UserRoleId");

                    b.HasIndex("RoleId")
                        .IsUnique();

                    b.HasIndex("UserId");

                    b.ToTable("UserRoles");
                });

            modelBuilder.Entity("Models.UserTask", b =>
                {
                    b.Property<int>("TaskId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("TaskId"));

                    b.Property<int>("AuthorId")
                        .HasColumnType("integer");

                    b.Property<int?>("ExecutorId")
                        .HasColumnType("integer");

                    b.Property<int?>("ExecutorId1")
                        .HasColumnType("integer");

                    b.Property<DateTime>("LastTaskTimeModification")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateOnly>("ReleaseVersion")
                        .HasColumnType("date");

                    b.Property<DateOnly>("SprintEndTime")
                        .HasColumnType("date");

                    b.Property<DateOnly>("SprintStartTime")
                        .HasColumnType("date");

                    b.Property<int?>("TaskAuthorAuthorId")
                        .HasColumnType("integer");

                    b.Property<string>("TaskDescription")
                        .HasMaxLength(10000)
                        .HasColumnType("character varying(10000)");

                    b.Property<string>("TaskName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("TaskPriority")
                        .HasColumnType("integer");

                    b.Property<int>("TaskSeverity")
                        .HasColumnType("integer");

                    b.Property<int>("TaskStatus")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(3);

                    b.Property<double>("TimeEstimation")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("double precision")
                        .HasDefaultValue(0.0);

                    b.Property<int?>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("TaskId");

                    b.HasIndex("AuthorId")
                        .IsUnique();

                    b.HasIndex("ExecutorId")
                        .IsUnique();

                    b.HasIndex("ExecutorId1");

                    b.HasIndex("TaskAuthorAuthorId");

                    b.HasIndex("UserId");

                    b.ToTable("Tasks");
                });

            modelBuilder.Entity("Models.Author", b =>
                {
                    b.HasOne("Models.User", "TaskAuthor")
                        .WithMany()
                        .HasForeignKey("TaskAuthorUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("TaskAuthor");
                });

            modelBuilder.Entity("Models.Executor", b =>
                {
                    b.HasOne("Models.User", "ExecutorTask")
                        .WithMany()
                        .HasForeignKey("ExecutorTaskUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ExecutorTask");
                });

            modelBuilder.Entity("Models.UserRoles", b =>
                {
                    b.HasOne("Models.Roles", "Roles")
                        .WithOne("UserRoles")
                        .HasForeignKey("Models.UserRoles", "RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Models.User", "User")
                        .WithMany("Roles")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Roles");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Models.UserTask", b =>
                {
                    b.HasOne("Models.Author", null)
                        .WithOne("UserTask")
                        .HasForeignKey("Models.UserTask", "AuthorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Models.Executor", null)
                        .WithOne("UserTask")
                        .HasForeignKey("Models.UserTask", "ExecutorId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("Models.Executor", "Executor")
                        .WithMany()
                        .HasForeignKey("ExecutorId1");

                    b.HasOne("Models.Author", "TaskAuthor")
                        .WithMany()
                        .HasForeignKey("TaskAuthorAuthorId");

                    b.HasOne("Models.User", null)
                        .WithMany("UserTasks")
                        .HasForeignKey("UserId");

                    b.Navigation("Executor");

                    b.Navigation("TaskAuthor");
                });

            modelBuilder.Entity("Models.Author", b =>
                {
                    b.Navigation("UserTask")
                        .IsRequired();
                });

            modelBuilder.Entity("Models.Executor", b =>
                {
                    b.Navigation("UserTask")
                        .IsRequired();
                });

            modelBuilder.Entity("Models.Roles", b =>
                {
                    b.Navigation("UserRoles")
                        .IsRequired();
                });

            modelBuilder.Entity("Models.User", b =>
                {
                    b.Navigation("Roles");

                    b.Navigation("UserTasks");
                });
#pragma warning restore 612, 618
        }
    }
}
